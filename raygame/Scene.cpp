...
{
    return m_UIElements.removeItem(m_UIElements.getItem(index));
}
...
void Scene::addItem(Actor* actor)
{
    ...
    for (int i = 0; i < actor->getTransform()->getChildCount(); i++)
    {
        addItem(actor->getTransform()->getChildren()[i]->getOwner());
    }
}
